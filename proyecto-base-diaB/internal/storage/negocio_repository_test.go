...
func TesTListar_NegocioEsta(t *testing.T) {
	repo := NewNegocioMemoria()
	p := models.Negocio{ID: 42, Nombre: "Manabita Crafts", Tipo: "artesania",
		Ciudad: "Manta", IdiomasHablados: []string{"es"}, Activo: true}
	err := repo.Guardar(p)
	if err != nil {
...
func TesTEliminar_NegocioInexistente(t *testing.T) {
	//repo := NewNegocioMemoria()
	base := models.Negocio{ID: 42, Nombre: "Manabita Crafts",
		Tipo: "artesania", Ciudad: "Manta",
		IdiomasHablados: []string{"es"}, Activo: true,
	}
	casos := []struct {
		nombre      string
	...
	for _, c := range casos {
		t.Run(c.nombre, func(t *testing.T) {
			...
			if !errors.Is(err, c.errEsperado) {
				t.Errorf("Esperaba error %v, obtuvo %v",
					c.errEsperado, err)
			}
		})
	}
